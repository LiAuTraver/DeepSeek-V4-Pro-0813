...
## Hierarchical Mermaid logical dataflow diagrams
...
This view intentionally contains only the four top-level execution blocks and the four outer
entry/state objects.
...
flowchart TD
    ...
    OV_BUF["$$\mathbf T^{\mathrm{buffer}}\in\mathbb Z^{B\times S_{\mathrm{total}}}\;[\mathrm{INT64}],\quad -1=\text{unfilled}$$"]
...
Here $\mathbf I_{\ell}^{\mathrm{CSA}}$ and $\mathbf I_{\ell}^{\mathrm{HCA}}$ denote the
history-index outputs of the two family-specific diagrams below.
...
This diagram applies only when $\ell\in\mathcal L_{\mathrm{CSA}}$. Its attention compressor and
lightning indexer are distinct modules with distinct FP32 projection weights, incomplete-state
pairs, and persistent output caches (compressed shared-KV versus compressed index keys).
...
flowchart TD
    ...
    C_POS -->|"$$p_0=0:\ \text{causal prefix boundary and physical offset }S;\quad p_0>0:\ \text{completed-history boundary and physical offset }W$$"| C_IHIST
...
This diagram applies only when $\ell\in\mathcal L_{\mathrm{HCA}}$. Each of its value and gate
projections emits one $d_h$-wide branch; it has no overlap transform, no lightning indexer, and
deterministically enumerates every causally visible completed compressed block.
...
This independent view preserves the final-head block's internal dataflow; its boundary inputs are
redeclared so the Mermaid block renders independently.
...
This independent view preserves the DSpark block's internal dataflow; its three boundary inputs are
redeclared so the Mermaid block renders independently. Dashed boundary edges show that
...
```mermaid
%%{init: {"flowchart": {"htmlLabels": true, "curve": "basis", "nodeSpacing": 24, "rankSpacing": 36}}}%%
flowchart TD
